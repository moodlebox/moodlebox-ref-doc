...
%% Suppress indentation and increase slightly the space between paragraphs
...
\title{MoodleBox building reference\\Version 4.11.1%
  \footnote{\url{https://doi.org/10.5281/zenodo.15763860}.}%
}
...
It is intended for developers or system administrators and provides background information on how a MoodleBox is built.
It is not meant to be an end-user documentation.

The MoodleBox end-user documentation is available on the MoodleBox website.\footnote{\url{https://moodlebox.net/}.}

The MoodleBox project is a free (as in speech and in beer) and open source project. The Ansible playbook source to build the MoodleBox image is provided under a AGPL v3.0 License. It is available on Github.\footnote{\url{https://github.com/moodlebox/moodlebox}.}

\subsection{Caveat}

The actual build of the MoodleBox image is done automatically using Ansible.\footnote{\url{https://www.ansible.com/}.}
...
\item Smartphone USB tethering: when connected via USB to a smartphone, MoodleBox can share the Internet connection provided by the smartphone to its wireless connected clients.
\item DHCP server for wireless connected clients.
...
\begin{itemize}
\item Moodle version 5.1.x in its basic configuration, with no content (no courses).
...
MoodleBox uses some of the great ideas from its first proof of concept by Christian Westphal\footnote{Christian Westphal, Académie de Strasbourg, author of "Enseigner en ligne avec Moodle", see \url{https://moodle.org/user/profile.php?id=1378197}.}, for which he deserves special thanks.

\subsection{License}

The present work is licensed under the \textsl{Creative Commons Attribution - Non Commercial - Share Alike  4.0 International license}.\footnote{\url{https://creativecommons.org/licenses/by-nc-sa/4.0/}.}


The MoodleBox source code is licensed under the \textsl{GNU Affero General Public License v3.0}.\footnote{\url{https://www.gnu.org/licenses/agpl-3.0.en.html}.}


The MoodleBox plugin for Moodle source code is licensed under the \textsl{GNU General Public License v3.0}.\footnote{\url{https://www.gnu.org/licenses/gpl-3.0.en.html}.}
...
For this process, use \textsl{Raspberry Pi Imager}.\footnote{Download Raspberry Pi Imager from \url{https://www.raspberrypi.com/software/}.}
...
Now click \textbf{Edit Settings} button to open OS customization.
...
Next, click \textbf{Yes} to apply OS customization settings when you write the image to the storage device.
...
If the hostname is changed, MoodleBox won't work correctly.
To prevent this issue, we check the hostname at first boot and fix it if needed.
This is done by copying the systemd unit file \lstinline{fix-hostname-once.service} below into \lstinline{/etc/systemd/system/}.
\lstinputlisting{files/fix-hostname-once-service}

This unit file makes use of the script \lstinline{fix-hostname-once.sh} below, to copy into \lstinline{/usr/local/sbin/}, with adequate permissions.
\lstinputlisting{files/fix-hostname-once-script}

Now let's enable the service
\begin{lstlisting}[language=bash]
$ sudo systemctl enable fix-hostname-once.service
\end{lstlisting}
...
An alternative firmware is available that has been tuned to maximize the number of clients in access point mode (AP) while still supporting client mode (STA).
...
We change some settings of the AP, in particular the DHCP settings, by adding two configuration files to \lstinline{dnsmasq} instance managed by NetworkManager.
The first one, named \lstinline{00-use-dnsmasq.conf}, has to be copied in \lstinline{/etc/NetworkManager/conf.d}.
Here's the content of file \lstinline{/etc/NetworkManager/dnsmasq-shared.d/00-dhcp.conf}:
\lstinputlisting{files/00-use-dnsmasq.conf}
The second file, named \lstinline{00-dhcp.conf}, has to be placed in \lstinline{/etc/NetworkManager/dnsmasq-shared.d/}.
Here's the content file \lstinline{/etc/NetworkManager/dnsmasq-shared.d/00-dhcp.conf}:
...
\subsection{Support USB tethering}

To support USB tethering out of the box for all devices, including those on iOS, we install \lstinline{usbmuxd}.
\begin{lstlisting}[language=bash]
...
\begin{lstlisting}[language=bash]
$ sudo apt install -y nginx php php-apcu php-mbstring php-curl php-fpm \
    php-gd php-intl php-mysql php-soap php-tidy php-xml php-yaml php-zip
\end{lstlisting}

\subsection{Configuration of Nginx and PHP}\label{ssec-nginx-php}

Nginx web server configuration is set in file \lstinline{/etc/nginx/sites-available/moodle}, which content should be like below.
\lstinputlisting[language=bash]{files/moodle}
...
To do this, we edit the two files \lstinline{/lib/systemd/system/nginx.service} and \lstinline{/lib/systemd/system/php8.4-fpm.service}, adding after the line \lstinline{[Service]} the line \lstinline{UMask=0002}, to get in both files something like
\begin{lstlisting}[language=bash]
...
[Service]
UMask=0002
...
...
\end{lstlisting}
We then edit the file \lstinline{/etc/php/8.4/fpm/pool.d/www.conf} to set the correct group ownership for PHP, replacing line \lstinline{group = www-data} with \lstinline{group = 1000}, so we have now
\begin{lstlisting}[language=bash]
...
user = www-data
group = 1000
...
We set the value of the variable \lstinline{date.timezone} to \lstinline{Europe/Zurich} in both of the files \lstinline{/etc/php/8.4/fpm/php.ini} and \lstinline{/etc/php/8.4/cli/php.ini}:
\begin{lstlisting}[language=bash]
date.timezone = Europe/Zurich
\end{lstlisting}

If needed, variables in \lstinline{/etc/php/8.4/fpm/pool.d/www.conf} can be tweaked, e.g. for performance gains. See page~\pageref{ssec-php-optimization} for examples.
...
found in any adequate documentation on SSL certificate generation.
% E.g. on \url{https://stackoverflow.com/a/10176685}
...
\end{lstlisting}
If needed, other variables in \lstinline{/etc/mysql/mariadb.conf.d/50-server.cnf} can be tweaked, e.g. for performance gains. See page~\pageref{ssec-mariadb-optimization} for examples.
...
We also install Composer, as Moodle requires it to manage its dependencies.
First, let's install those two softwares.
\begin{lstlisting}[language=bash]
...
\begin{lstlisting}[language=bash]
$ sudo git clone --depth=1 -b MOODLE_501_STABLE \
    https://github.com/moodle/moodle.git /var/www/moodle
\end{lstlisting}

We now install Moodle dependencies using Composer:
\begin{lstlisting}[language=bash]
$ cd /var/www/moodle
...
\subsubsection{Moodle router}

We've already configured the Moodle router, with the following lines in the Nginx configuration file \lstinline{/etc/nginx/sites-available/default}, inside \lstinline{server} block (see page~\pageref{ssec-nginx-php}):
\begin{lstlisting}[language=bash]
location / {
    try_files \$uri \$uri/ /r.php;
}
\end{lstlisting}
We mark the Moodle router as configured in \lstinline{/var/www/moodle/config.php}.
\begin{lstlisting}[language=php]
...
Moodle cron should be launched every minute, and, as MoodleBox is offline most of the time and doesn't have a mail server, cron should send no mail.
\begin{lstlisting}[language=bash]
$ sudo crontab -e
\end{lstlisting}
We add the following lines to the crontab
\begin{lstlisting}[language=bash]
MAILTO=""
* * * * * nice -n10 /usr/bin/php /var/www/moodle/admin/cli/cron.php
\end{lstlisting}

\section{MoodleBox plugin for Moodle}

MoodleBox plugin\footnote{MoodleBox plugin source code is available at \url{https://github.com/moodlebox/moodle-tool_moodlebox}.} provides monitoring of the MoodleBox and enables to manage various MoodleBox settings, such as password change, wireless access settings, etc.

It supports Raspberry Pi hardware only and needs some prerequisites to work correctly.
...
$ sudo -u www-data -g moodlebox git clone --depth=1 \
    ...
    /var/www/moodle/public/admin/tool/moodlebox
\end{lstlisting}
...
\subsection{Completion of the installation}
...
\begin{lstlisting}[language=bash]
$ cd /var/www/moodle/public/admin/tool/moodlebox
$ sudo touch .reboot-server .shutdown-server .set-server-datetime \
    .newpassword .wifisettings .resize-partition
$ sudo chown -R www-data:moodlebox /var/www/moodle/public/admin/tool/moodlebox
$ sudo chmod -R ug+w,o-w /var/www/moodle/public/admin/tool/moodlebox
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
success_rootexec = /usr/local/sbin/fix-mount-permissions.sh
\end{lstlisting}
We copy the script file \lstinline{fix-mount-permissions.sh} to \lstinline{/usr/local/sbin/}.
...
\begin{lstlisting}[language=bash]
$ sudo git clone --depth=1 -b 3.2.2 https://github.com/mathjax/MathJax.git /var/www/moodle/public/lib/mathjax
$ sudo chown -R www-data:moodlebox /var/www/moodle/public/lib/mathjax
$ sudo chmod -R ug+w,o-w /var/www/moodle/public/lib/mathjax
\end{lstlisting}
...
We make sure that Moodle debug messages display is disabled, by visiting \emph{Site administration > Development > Debugging}, unchecking the checkbox \emph{Display debug messages} if needed.
...
\begin{lstlisting}[language=bash]
$ sudo wget -c https://www.adminer.org/latest.php -O /var/www/moodle/public/adminer.php
...
To make the MoodleBox more comfortable to use, it is necessary to take care of its optimization.
We configure Moodle's cache, as well as its management of file uploads and downloads.

\subsection{Setup RAM disks for specific Moodle directories}
...
\subsection{MariaDB optimization}\label{ssec-mariadb-optimization}

We configure a few MariaDB variables in \lstinline{/etc/mysql/mariadb.conf.d/50-server.cnf}:
\begin{lstlisting}[language=bash]
skip-name-resolve
query_cache_size      = 4M
\end{lstlisting}

\subsection{PHP optimization}\label{ssec-php-optimization}

For a better performance, we tweak the value of a PHP variable in the file \lstinline{/etc/php/8.4/fpm/pool.d/www.conf}:
\begin{lstlisting}[language=bash]
pm.max_requests = 50
\end{lstlisting}
