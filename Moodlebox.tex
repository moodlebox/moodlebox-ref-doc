...
\title{MoodleBox building reference, version 4.6.0 (unreleased)\footnote{This work is licensed under a \href{http://creativecommons.org/licenses/by-nc-sa/4.0/}{Creative Commons Attribution - Non Commercial - Share Alike  4.0 International} license.}}
...
\begin{itemize}
\item Moodle version 4.1.x in its basic configuration, with no content (no courses).
...
We edit \lstinline{/etc/dnsmasq.conf} to have this content. Note the static IP address set up on page~\pageref{ssec-static-ip}:
...
\begin{lstlisting}[language=bash]
$ sudo dpkg –i nodogsplash_5.0.1-1_arm64.deb
\end{lstlisting}
...
$ sudo apt-get install -y nginx php7.4-fpm php7.4-cli php7.4-common \
    ...
    php7.4-curl php7.4-gd php7.4-intl php7.4-soap php7.4-mysql \
    php7.4-xml php7.4-zip php7.4 php-apcu
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ sudo git clone --depth=1 -b MOODLE_401_STABLE \
