...
\item Set time zone and WLAN country. MoodleBox default settings are respectively \lstinline{Europe/Zurich} and \lstinline{CH};
...
%\footnote{This section and the next one are based on \url{https://frillip.com/using-your-raspberry-pi-3-as-a-wifi-access-point-with-hostapd/}.}} %% Link is dead

\subsection{Network interfaces configuration}

MoodleBox gets dynamically its IP address on the ethernet interface \lstinline{eth0} via DHCP.
With its access point feature, it will also act as a DHCP provider on its wireless interface \lstinline{wlan0}.
We force then a static IP address \lstinline{10.0.0.1} on \lstinline{wlan0} interface.

To this end, we add at the very end of file\footnote{These lines should however be inserted \textbf{before} any \emph{interface} line added in the file.} \lstinline{/etc/dhcpcd.conf} the lines
\begin{lstlisting}[language=bash]
interface wlan0
...
\subsection{Access point configuration (\lstinline{hostapd})}
...
We get an error message (expected as the configuration is not finished yet); this message should end with \lstinline{wlan0: AP-ENABLED}.
...
TODO: This has changed as of Bullseye. Change and document if needed.
...
\subsection{DHCP and DNS server configuration (\lstinline{dnsmasq})}

DHCP and DNS server configuration on \lstinline{wlan0} interface are provided through \lstinline{dnsmasq}\footnote{DNS configuration is based on \url{https://www.linux.com/learn/dnsmasq-easy-lan-name-services}.}. We edit  \lstinline{/etc/dnsmasq.conf} to have this content:
