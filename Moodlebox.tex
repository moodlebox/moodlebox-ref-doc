...
We can now install the captive portal, based on free software Nodogsplash\footnote{https://nodogsplashdocs.readthedocs.io/}.
...
We can now edit Nodogsplash splash page at our convenience. The files to edit are located in folder \lstinline{/etc/nodogsplash/htdocs/}.
...
\iffalse
\begin{verification}
With a computer connected to the MoodleBox wireless network, load URL \url{http://moodlebox.home/} in your favorite web browser. The web page "Welcome to nginx on Debian!" Should be displayed.
\end{verification}
\fi
...
Last \lstinline{fastcgi_param} line, with line \lstinline{client_max_body_size}, increases to 50~MB the max upload file size, as well as script maximum execution time to~300~s. It also sets \lstinline{max_input_vars} to~5000.
...
\end{lstlisting}
then open URL \url{http://moodlebox.home/info.php} in your browser. The long PHP settings page should appear, with the \lstinline{upload_max_filesize}, \lstinline{post_max_size} and \lstinline{max_input_vars} variables correctly set.
\end{verification}
\fi
...
We install now MariaDB package. PHP MariaDB support was already installed with PHP (see section~\ref{ssec-nginx-php-installation}).
\begin{lstlisting}[language=bash]
$ sudo apt-get install -y mariadb-server
\end{lstlisting}

During the installation, we define the password of the main user \emph{moodlebox} of the database. For this installation, we set the password to \emph{Moodlebox4\$}.

\iffalse
\begin{verification}
Reload URL \url{http://moodlebox.home/info.php}. A new section \emph{MySQL} should appear in the page.
\end{verification}
\fi
...
We can now download Moodle; we do this with Git in order to facilitate future updates. We first install Git.
\begin{lstlisting}[language=bash]
...
Moodle data and several cache directories are now created, with adequate permissions. We also set the correct permissions to Moodle source directory and set SGID permission on Moodle data directory:
\begin{lstlisting}[language=bash]
...
We can now launch Moodle installation, either by loading URL \url{http://moodlebox.home/} in your browser and follow the instructions on screen, or by using Moodle command line interface\footnote{See \url{https://docs.moodle.org/311/en/Installing_Moodle\#Command_line_installer}.}. For MoodleBox, the administrator account is defined with \emph{moodlebox} as username and the password \emph{Moodlebox4\$}.

The installation takes more than 10 minutes. So be patient!
